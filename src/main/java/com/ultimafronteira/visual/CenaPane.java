...
public class CenaPane extends Pane {
    ...
    private void desenharBarrasDeStatus(GraphicsContext gc, Personagem jogador, double pX, double pY, double pWidth) {
        ...
        double barHeight = 12;
        double spacing = 4;
        double startY = pY - (barHeight + spacing) * 5 - 25;


        desenharTextoDeBuff(gc, jogador.getNome(), pX + barWidth / 2, startY - 15, Color.WHITE);

        desenharBarra(gc, "Vida", jogador.getVida(), jogador.getVidaMaxima(), pX, startY, barWidth, barHeight, Color.LIMEGREEN);
        desenharBarra(gc, "Energia", jogador.getEnergia(), jogador.getEnergiaMaxima(), pX, startY + (barHeight + spacing), barWidth, barHeight, Color.DEEPSKYBLUE);
        desenharBarra(gc, "Fome", jogador.getFome(), jogador.getFomeMaxima(), pX, startY + (barHeight + spacing) * 2, barWidth, barHeight, Color.GOLD);
        desenharBarra(gc, "Sede", jogador.getSede(), jogador.getSedeMaxima(), pX, startY + (barHeight + spacing) * 3, barWidth, barHeight, Color.ROYALBLUE);
        desenharBarra(gc, "Sanidade", jogador.getSanidade(), jogador.getSanidadeMaxima(), pX, startY + (barHeight + spacing) * 4, barWidth, barHeight, Color.MEDIUMORCHID);
    }

    private void desenharTextoDeBuff(GraphicsContext gc, String texto, double x, double y, Color cor) {
        gc.save();
