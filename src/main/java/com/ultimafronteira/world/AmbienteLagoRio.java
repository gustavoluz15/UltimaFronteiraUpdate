...
=======
    private boolean possibilidadeDePesca; // Atributo com 'i'
    private boolean terrenoLamacento;

    public AmbienteLagoRio(String nome, String descricao) {
        super(nome, descricao, "Moderada (risco de afogamento)", "Variável, geralmente úmido");
        this.aguaAbundante = true;
        this.possibilidadeDePesca = true; // Atribuído corretamente aqui
...
=======
        adicionarRecurso(new Agua("Água do Rio", 0.0, -1, 15, "Potável", 0.0));
        if (this.possibilidadeDePesca) { // Usado corretamente aqui
            adicionarRecurso(new Alimento("Peixe Pequeno", 0.4, 1, 12, "Carne de Peixe"));
        }
        adicionarRecurso(new Material("Juncos", 0.1, "Fibra Vegetal", 2));
    }

    public boolean isAguaAbundante() { return aguaAbundante; }
    public boolean isPossibilidadeDePesca() { return this.possibilidadeDePesca; } // CORRIGIDO AQUI para this.possibilidadeDePesca
>>>>>>> f7209b58956d6d2fcbc7f29dfcf96ffd5093dbcd
...
=======
            if (this.possibilidadeDePesca && Math.random() < (0.4 + (jogador.temHabilidade("Rastreamento Aguçado") ? 0.1 : 0.0) )) { // Usado corretamente aqui
>>>>>>> f7209b58956d6d2fcbc7f29dfcf96ffd5093dbcd
