...
=======
        super(nome, descricao, "Moderada (risco de desmoronamento)", "Variável, parcialmente abrigado");
...
=======
        adicionarRecurso(new Material("Metal Retorcido", 1.5, "Metal", 5));
        adicionarRecurso(new Material("Tijolos Quebrados", 2.0, "Construção", 3));
        if (Math.random() < 0.2) {
            adicionarRecurso(new Ferramenta("Ferramenta Enferrujada", 1.0, 3, "Variada", 2));
        }
        if (Math.random() < 0.15) {
            adicionarRecurso(new Alimento("Enlatado Antigo", 0.8, 1, 25, "Enlatado"));
>>>>>>> f7209b58956d6d2fcbc7f29dfcf96ffd5093dbcd
        }
    }
...
=======
        double chanceBaseRecurso = 0.55; // Chance original
        if (jogador.temHabilidade("Rastreamento Aguçado") || jogador.temHabilidade("Mãos Leves")) { // Ladrão também poderia ser bom aqui
>>>>>>> f7209b58956d6d2fcbc7f29dfcf96ffd5093dbcd
